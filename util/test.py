...
function_str = "(镇静.催眠|镇静.镇痛|镇静.?催眠.急性乙醇戒断|镇静催眠、急性酒精戒断|抗焦虑.镇静催眠|镇痛麻醉|手术后镇痛|分娩镇痛|镇静|催眠|镇痛|抗恐惧|抗癫痫.抗惊厥|小儿惊厥|癫痫持续状态和严重复发性癫痫|癫痫持续状态|一般性失眠|抗?癫痫|抗?失眠|抗?惊厥|抗?焦虑|乙醇戒断|基础麻醉或静脉全麻" \
               ...
               "|硬膜外腔阻滞麻?醉?|手?术中维持肌松|维持肌松|电休克|快?速?气管插管|维持肌肉松弛|半去极化肌松药的拮抗|青光眼|呕吐|精神分裂症|肾功能不全|肝功能不全|遗尿症|抗高胆红素血症|缓释制剂|治疗心律失常|拮抗东芨着碱中毒" \
...
#按年龄、作用、给药方式+判断是否有单次服用剂量来切分句子  ……的患者是可以匹配到所有的者，但是要统计年龄的时候就不够详细
...
import re
patr_test = re.compile("(局部浸润麻?醉?)")
str = "局部浸润麻醉"
match_test = patr_test.findall(str)
f = patr_test.finditer(str)
if f:
    inds = [(i.start(),i.end()) for i in f]
    print(inds)
print(match_test)
