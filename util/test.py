...
function_str = "(镇静|催眠|镇静.催眠|抗恐惧|抗?癫痫|抗?失眠|一般性失眠|抗癫痫.抗惊厥|抗?惊厥|小儿惊厥|抗?焦虑|抗焦虑.镇静催眠|镇静.催眠.急性乙醇戒断|镇静催眠、急性酒精戒断|乙醇戒断|基础麻醉或静脉全麻|癫痫持续状态和严重复发性癫痫" \
               ...
               "|硬膜外腔阻滞|神经阻滞|手术后镇痛|分娩镇痛|手?术中维持肌松|维持肌松|电休克|气管插管|维持肌肉松弛|半去极化肌松药的拮抗|青光眼|呕吐|精神分裂症|肾功能不全|肝功能不全|遗尿症|抗高胆红素血症|缓释制剂)"

age_str = "(肝、肾功能损害者|老年人|成人|老年和体弱或肝功能不全患者|特殊人群：严重肝损患者|老年或虚弱的患者|老年或虚弱者|年老体弱者|老年、重病和肝功能受损患者|重症患者|肝、肾疾病患者" \
          "|老年、女性、非吸烟、有低血压倾向、严重肾功能 损害或中度肝功能损害患者|新生儿)"
...
#100-200页内分号切分个例
semicolon_12_zd = "([;；]快速气管插管|[;；]癫痫持续状态|[;；]麻醉前用药|[;；]术后应用|[;；]拮抗东芨着碱中毒|[;；]癫痫持续状态|[;；]牙科|[;；]肋间神经|[;；]宫颈旁浸润|[;；]椎旁脊神经阻滞" \
                  "|[;；]阴部神经|药物诱发的锥体外系反应|药物诿发的锥体外系反应|注入蛛网膜下隙|硬膜外PCA|，背景输注量|重度疼痛|如不能控制|用作胶原酶合成抑制剂时|一过性失眠|或按体表面积|用量视患者的耐受情况)"
#100-200页内无分号切分个例
...
#按分号切分句子
# import re
def get_concat_str(search_string,param_list=None):
    concat_string = ""
    admin_search = take_patr.search(search_string)
    if param_list:
        begin_str = ''.join(param_list)
...
def get_semi_cut(str):
    ...
    if semiconlon_patr.search(str):
        result = re.split(semicolon_str,str)
    else:
    ...
    if semi_result:
        if len_semi >1:
            ...
            #有（1）标号
            if b_match:
                # 有（1）标号，有①标号，直接拼接(1)和①标号之间的内容，①标号中判断是否有服用方式和作用，服用方式拼接，作用在断句中有则不需拼接，没有要拼接
                if cir_match:
                    concat_strb = str[:cir_match.start()]#(1)……
                    concat_strcir = str[cir_match.start():]#①……
                    param_str = [concat_strb,cir_match.group()]
                    concat_string = get_concat_str(concat_strcir,param_str)
                # 有（1）标号，没有①标号，判断（1）句是否有服用方式和作用，服用方式需拼接，作用在断句中有则不需拼接，没有要拼接
                else:
                    param_str = [b_match.group()]
                    concat_string = get_concat_str(concat_str, param_str)
            #无（1）标号
            else:
                # 没有(1)标号，有①标号，前面有文字的直接拼接，①标号后的文字中判断是否有服用方式和作用，服用方式拼接，作用在断句中有则不需拼接，没有要拼接
                if cir_match:
                    ...
                    concat_string = get_concat_str(after_cir_str, param_str)
                # 没有(1)标号，没有①标号，判断本句有没有服用方式和作用，没有的话判断前面第一句（下标0）是否有服用方式和作用，本句有则拼接，没有不管
                else:
                    param_str = []
                    concat_string = get_concat_str(concat_str, param_str)
...
# semi_cut: ['（1）口服成人抗癫痫一般一次0.03g，一日3次', '（1）口服成人。抗癫痫；或0.09g睡前顿服。极量一次0.25g。一日0.5g。']
...
print("semi_cut:",get_semi_cut(semi_str3))
